...
def non_max_suppression(prediction, conf_thres=0.1, iou_thres=0.6, merge=False, classes=None, agnostic=False):
    ...
    Returns:
         detections with shape: nx7 (x1, y1, x2, y2, conf, cls, depth)
    """

    nc = prediction[0].shape[1] - 6  # subtract 5 + 1 (depth)
    ...
    t = time.time()
    output = [torch.zeros(0, 7)] * prediction.shape[0]
    for xi, x in enumerate(prediction):
        ...
        # Separate depth
        depth = x[:, -1].unsqueeze(1)  # last column
        x = x[:, :-1]  # remove depth
        ...
        if multi_label:
            ...
            x = torch.cat((box[i], x[i, j + 5, None], j[:, None].float(), depth[i]), 1)
        else:
            conf, j = x[:, 5:].max(1, keepdim=True)
            mask = conf.view(-1) > conf_thres
            x = torch.cat((box, conf, j.float(), depth), 1)[mask]
