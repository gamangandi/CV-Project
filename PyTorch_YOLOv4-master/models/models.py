    ...
    for i, mdef in enumerate(module_defs):
        ...
        if mdef['type'] == 'convolutional':
            ...
            try:
                ...
                #bias[:, 4] += -4.5  # obj
                bias.data[:, 4] += math.log(8 / (640 / stride[yolo_index]) ** 2)  # objectness
                bias.data[:, 6:] += math.log(0.6 / (modules.nc - 0.99))  # skip index 5 (depth), init class probs
...
class YOLOLayer(nn.Module):
    def __init__(self, anchors, nc, img_size, yolo_index, layers, stride):
        ...
        self.no = nc + 6  # number of outputs (85)
    ...
    def forward(self, p, out):
        ...
        if self.training:
            ...
            #io[..., :4] *= self.stride
            #torch.sigmoid_(io[..., 4:])
            # scale depth (optional - assume depth is in 0-10m)
            io[..., 5] = io[..., 5] * 10.0
...
def load_darknet_weights(self, weights, cutoff=-4):
